...
TaskHandle_t receive1_task_handler;
TaskHandle_t receive2_task_handler;
...
void send_task(void *pvParameters) {
    ...
    while(1) {
        if(HAL_GPIO_ReadPin(KEY0_PORT, KEY0_PIN) == GPIO_PIN_RESET) {
            xTaskNotify(receive1_task_handler, 1, eSetValueWithOverwrite);
        }
        if(HAL_GPIO_ReadPin(KEY1_PORT, KEY1_PIN) == GPIO_PIN_RESET) {
            xTaskNotify(receive2_task_handler, 2, eSetValueWithOverwrite);
        }
        vTaskDelay(200);
    }
}
...
void receive1_task(void *pvParameters) {
    ...
    while(1) {
        res = xTaskNotifyWait(0, ULONG_MAX, &receive_value, portMAX_DELAY);
        if(res == pdPASS) {
            my_printf(&uart1_handler, "key0 pressed, msg is %d\r\n", receive_value);
        }
        vTaskDelay(1);
    }
}
...
void receive2_task(void *pvParameters) {
    ...
    while(1) {
        res = xTaskNotifyWait(0, ULONG_MAX, &receive_value, portMAX_DELAY);
        if(res == pdPASS) {
            my_printf(&uart1_handler, "key1 pressed, msg is %d\r\n", receive_value);
        }
        vTaskDelay(1);
    }
}
...
void start_task(void *pvParameters)
{
    ...
    x_return = xTaskCreate(receive1_task, "receive1_task", 256, NULL, 4, &receive1_task_handler);
    my_printf(&uart1_handler, "create %s %s\r\n", "receive1_task", (x_return == pdPASS)? "success" : "failed");
    x_return = xTaskCreate(receive2_task, "receive2_task", 256, NULL, 4, &receive2_task_handler);
    my_printf(&uart1_handler, "create %s %s\r\n", "receive2_task", (x_return == pdPASS)? "success" : "failed");
    vTaskDelete(NULL);
    taskEXIT_CRITICAL();
}
